...
void Shader_Generic_Base::build() {
    printf("renderer: building shader '%s'\n", name());
    auto vsh = FromStringLoadShader<GL_VERTEX_SHADER>(generic_vsh_glsl, defines());
    auto fsh = FromStringLoadShader<GL_FRAGMENT_SHADER>(generic_fsh_glsl, defines());
...
void Shader_Generic_Base::linkUniforms() {
...
char const *Shader_Generic::name() const {
    return "Generic";
}

Shader_Define_List Shader_Generic::defines() const {
    return {};
}

// ========================== Shader_Generic_With_Vertex_Colors

char const *Shader_Generic_With_Vertex_Colors::name() const {
    return "Generic (with vertex colors)";
}

Shader_Define_List Shader_Generic_With_Vertex_Colors::defines() const {
    return { { "GENERIC_SHADER_WITH_VERTEX_COLORS", "1" } };
}

// ========================== Shader_Generic_Textured_Unlit

char const *Shader_Generic_Textured_Unlit::name() const {
    return "Generic (textured, unlit)";
}

Shader_Define_List Shader_Generic_Textured_Unlit::defines() const {
    return { { "TEXTURED", "1" } };
}

void Shader_Generic_Textured_Unlit::linkUniforms() {
...
char const *Shader_Generic_Textured_Lit::name() const {
    return "Generic (textured, lit)";
}

Shader_Define_List Shader_Generic_Textured_Lit::defines() const {
    return { { "TEXTURED", "1" }, { "LIT", "1" } };
}

void Shader_Generic_Textured_Lit::linkUniforms() {
    Shader_Generic::linkUniforms();
    ...
    _locViewPosition = gl::Uniform_Location<glm::vec3>(program(), "viewPosition");
}
